...
class Client {
    ...
    this._channelMemberJoinedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberJoined)
          this.onMemberJoined({ channelSid, member: new Member(member) })
      }
    )
    ...
    this._channelMemberChangedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberChanged)
          this.onMemberChanged({ channelSid, member: new Member(member) })
      }
    )
    ...
    this._channelMemberLeftSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onMemberLeft)
          this.onMemberLeft({ channelSid, member: new Member(member) })
      }
    )
    ...
    this._typingChannelStartedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onTypingStarted)
          this.onTypingStarted({ channelSid, member: new Member(member) })
      }
    )
    ...
    this._typingChannelEndedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ channelSid, member }) => {
        if (this.onTypingEnded)
          this.onTypingEnded({ channelSid, member: new Member(member) })
      }
    )
    ...
    this._userInfoUpdatedSubscription = NativeAppEventEmitter.addListener(
      ...
      ({ updated, userInfo }) => {
        ...
        if (this.onUserInfoUpdated)
          this.onUserInfoUpdated({ updated, userInfo: this.userInfo })
      }
    )
  ...
  getClientChannels() {
    return TwilioChatClient.getChannels().then(

...
public class RCTTwilioChatClient extends ReactContextBaseJavaModule implements ChatClientListener {
    ...
    public void createClient(String token, ReadableMap props, final Promise promise) {
        ...
        ChatClient.Properties.Builder builder = new ChatClient.Properties.Builder().setRegion("us1");
        ChatClient.create(reactContext, token, builder.createProperties(), new CallbackListener<ChatClient>() {
            @Override

            public void onError(ErrorInfo errorInfo) {
                Log.d("CAREA", "onError one = " + errorInfo);
                super.onError(errorInfo);
                Log.d("CAREA", "onError two");
                ...
                Log.d("CAREA", "onError three");

            }

            @Override
            public void onSuccess(ChatClient twilioChatClient) {
                Log.d("CAREA", "onSuccess one");
...
//        tmp.client.handleNotification(map);
    ...
    // Volodymyr Custom methods
//    @ReactMethod
//    public void getUserChannels(final Promise promise) {
//        RCTTwilioChatClient tmp = RCTTwilioChatClient.getInstance();
//        tmp.client.getChannels(new CallbackListener<Paginator<ChannelDescriptor>>() {
//            @Override
...
//        RCTTwilioChatClient tmp = RCTTwilioChatClient.getInstance();
//        Log.d("OLD", "getChannels" + tmp);
//        tmp.getChannels(promise);
//    }

@ReactMethod
public void getChannels(final Promise promise) {
    Log.d("RCTTwilioChatClient", "getUserChannels " + client);
    client.getChannels().getUserChannelsList(new CallbackListener<Paginator<ChannelDescriptor>>() {
        @Override
        public void onError(final ErrorInfo errorInfo) {
            super.onError(errorInfo);
            promise.reject("get-user-channels-error", "Error occurred while attempting to getUserChannels.");
